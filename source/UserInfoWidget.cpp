...
void UserInfoWidget::initUI()
{
    ...
    // 设置个性签名
    ui->personal_sign_label->setText(_user_info.sign);
    QString user_info = QString("sex: %1, address: %2")
            .arg(_user_info.gender == 1 ? "Male" : "Female")
...
void UserInfoWidget::onChatButtonClicked(){
    emit chatWithUser(_user_info.uid);
}